...
template <typename T> SharedArray<T> *SharedArray<T>::CreateOne(size_t _size, bool clearMemory)
{
    Debug::OnBeforeAllocate();
    T *arrayPtr =
        clearMemory ? static_cast<T *>(calloc(_size, sizeof(T))) : static_cast<T *>(malloc(_size * sizeof(T)));
...
template <typename T> std::span<T> const &SharedArray<T>::GetArray() const
{
    return m_view;
}
